...
public class FaceRecognitionService {
    ...
    public void trainRecognizer() {
        try {
            ...
            // Collect valid face images first, then build the labels Mat
            ...
            if (images.size() > 0) {
                ...
                for (int i = 0; i < labelList.size(); i++) {
                    labels.ptr(i).putInt(labelList.get(i));
                }
                recognizer.train(images, labels);
                System.out.println("✅ Recognizer trained with " + images.size() + " faces!");
            } else {
                System.out.println("ℹ️ No valid face images found for training.");
            }
